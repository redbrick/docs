...
- Each admin will recieve a local account on each machine that will be in the root group. This allows you to log on if [LDAP](services/ldap) goes down.
...
- Don't change the root account information!
...
Do not put personal customisations into the default root account setup, remember other people have to use it.
...
All downtime must be scheduled and notified to the members well in advance by means of motd & an announcement in the [Discord](https://discord.redbrick.dcu.ie) server. If it's really important, a mail to announce-redbrick and socials post may be necessary.
